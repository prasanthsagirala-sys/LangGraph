...
from RAG_Main import setup_pipeline_and_query
...
@tool
def rag_tool(pdf_path: str, query: str) -> str:
    """
    Perform RAG (Retrieval-Augmented Generation) on the given PDF document.
    """
    ans = setup_pipeline_and_query(pdf_path, query)
    return ans
...
# out = chatbot.invoke(
#     {'messages': [SystemMessage(content = r'You are a helpful assistant. Use the pdf document from path (C:\Users\sgp\Desktop\GenAI\LangGraph\08 Chatbot with RAG as Tool\dl-curriculum.pdf) if required'),
#                   HumanMessage(content = 'what are the layers discussed in the document?')]}, 
#     config = {'configurable':{'thread_id':'thread-1'}}
# )
